...
/*
	List
	- 자료를 순차적으로 나열한 자료 구조(배열과 비슷)
	- 인덱스가 존재함
	- 인덱스로 순서가 구분되기 때문에 중복 데이터 저장 가능
	- 인터페이스. 때문에 객체로 생성이 불가능. 다형성으로 이용할 수 있음
...
public class ListEx {
	...
	/*	
		...
		List<자료형> list = new ArrayList(3) <>로 타입을 정해주지 않아
		노랗게 경고가 발생.
		노란색을 없애고 싶다면 List<Object>을 넣어서 사용할 수 있음
		Object = String, Integer 등 모두 사용 가능
		*/
		...
		//3을 초과해서 크기가 자동으로 늘어나기 때문에 에러 발생 x
		list.add(false);
